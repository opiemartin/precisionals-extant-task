...
# . Visual check data
ggplot(D[D$TIME < 13.5, ], aes(TIME, TOTAL, by = ID)) +
  geom_line(alpha = .1)
...
D <- D[!D$TIME > 120, ]
# D <- D[!D$DIFF >= 5, ] # Option to reduce noise
...
D$SITE <- D$Site
D <- D[(D$TIME < 27.5) & (D$SITE %in% c("King's", "Utrecht") == F), ]

plot(D$TIME, D$TIME_12)
...
# . Average progression of 0.982 pts / mo
summary(m)
